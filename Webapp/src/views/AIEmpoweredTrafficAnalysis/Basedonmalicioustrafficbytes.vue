...
const documentUrl = ref("https://yu5fu9ktnt.feishu.cn/docx/IBa0d0UDVoy2KExMUxlct1kznCL?from=from_copylink");
...
const downZip = async () =>{
  ...
  try {
    const studentCondition = new Student2Resource(studentid, 50003, '', '', Date.now());
...
const uploadpath = ref<string>(`/${studentid}/50003/result.csv`)
...
const scoreCsv = async () => {
  ...
  try {
    // 调用评分API
    const commit = new Commit(0,studentid, 50003, 0, `studentsdata/${studentid}/50003/result.csv`, getCurrentTime(), false);
