...
const downPcap = async () =>{
  const studentCondition = new Student2Resource(studentId.value, 50006, '', '', Date.now());
...
const studentid = getCookie('studentId')
const uploadpath = ref<string>(`/${studentid}/50006/result.csv`)
...
const scoreCsv = async () => {
  ...
  try {
    // 调用评分API
    const commit = new Commit(0,studentid, 50006, 0, `studentsdata/${studentid}/50006/result.csv`, getCurrentTime(), false);
