...
const documentUrl = ref("https://yu5fu9ktnt.feishu.cn/docx/KqhcdaR4QoC1C6xlt2Xchpx1njn?from=from_copylink");
...
const downZip = async () =>{
  ...
  try {
    const studentCondition = new Student2Resource(studentid, 50005, '', '', Date.now());
...
const uploadpath = ref<string>(`/${studentid}/50005/result.csv`)
...
const scoreCsv = async () => {
  ...
  try {
    // 调用评分API
    const commit = new Commit(0,studentid, 50005, 0, `studentsdata/${studentid}/50005/result.csv`, getCurrentTime(), false);
...
const goToLastPage = async () => {
  await router.push(`/experiment/50003?title=子任务2：基于流量字节的恶意流量检测`);
}

const goToNextPage = async () => {
  await router.push(`/experiment/50006?title=子任务2：使用混合输入进行加密流量分类`);
}
</script>
