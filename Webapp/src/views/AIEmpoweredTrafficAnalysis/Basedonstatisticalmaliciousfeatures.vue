...
const documentUrl = ref("https://yu5fu9ktnt.feishu.cn/docx/BcWDdH8M3odcFRximoSc91KFnHg?from=from_copylink");
...
const downZip = async () =>{
  ...
  try {
    const studentCondition = new Student2Resource(studentid, 50002, '', '', Date.now());
...
const uploadpath = ref<string>(`/${studentid}/50002/result.csv`)
...
const scoreCsv = async () => {
  ...
  try {
    // 调用评分API
    const commit = new Commit(0,studentid, 50002, 0, `studentsdata/${studentid}/50002/result.csv`, getCurrentTime(), false);
