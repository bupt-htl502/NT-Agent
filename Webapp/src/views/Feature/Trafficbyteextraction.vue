...
const downPcap = async () =>{
  const studentCondition = new Student2Resource(studentId.value, 40010, '', '', Date.now());
...
const studentid = getCookie('studentId')
const uploadpath = ref<string>(`/${studentid}/40010/result.csv`)
...
const scoreCsv = async () => {
  ...
  try {
    // 调用评分API
    const commit = new Commit(0,studentid, 40010, 0, `studentsdata/${studentid}/40010/result.csv`, getCurrentTime(), false);
