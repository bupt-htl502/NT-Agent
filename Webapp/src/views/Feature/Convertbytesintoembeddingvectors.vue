...
const downPcap = async () =>{
  const studentCondition = new Student2Resource(studentId.value, 40012, '', '', Date.now());
...
const studentid = getCookie('studentId')
const uploadpath = ref<string>(`/${studentid}/40012/result.csv`)
...
const scoreCsv = async () => {
  ...
  try {
    // 调用评分API
    const commit = new Commit(0,studentid, 40012, 0, `studentsdata/${studentid}/40012/result.csv`, getCurrentTime(), false);
