...
const documentUrl = ref("https://yu5fu9ktnt.feishu.cn/docx/FjUjdT2D6o5PojxoPPHcthl3nc5?from=from_copylink");
...
const downZip = async () =>{
  ...
  try {
    const studentCondition = new Student2Resource(studentid, 40008, '', '', Date.now());
...
const uploadpath = ref<string>(`/${studentid}/40008/result.csv`)
...
const scoreCsv = async () => {
  ...
  try {
    // 调用评分API
    const commit = new Commit(0,studentid, 40008, 0, `studentsdata/${studentid}/40008/result.csv`, getCurrentTime(), false);
