...
const documentUrl = ref("https://yu5fu9ktnt.feishu.cn/docx/UI86dCGz9oKuKAxuImhcyJbTnnd?from=from_copylink");
...
const downZip = async () => {
  await downloadExperimentData(studentid as number, 40002); // 场景ID可动态传入
};
...
const uploadpath = ref<string>(`/${studentid}/40002/result.zip`)
...
const scoreZip = async () => {
  ...
  try {
    // 调用评分API
    const commit = new Commit(0,studentid, 40002, 0, `studentsdata/${studentid}/40002/result.zip`, getCurrentTime(), false);
