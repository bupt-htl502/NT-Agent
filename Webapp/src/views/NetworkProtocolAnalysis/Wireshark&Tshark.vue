...
const documentUrl = ref("https://yu5fu9ktnt.feishu.cn/docx/Moy0dNesyoWQ0zxPY5ockgqSnpL?from=from_copylink");
...
const studentid = getCookie('studentId')
const uploadpath = ref<string>(`/${studentid}/10002/result.csv`)
...
const scoreCsv = async () => {
  ...
  try {
    // 调用评分API
    const commit = new Commit(0,studentid, 10002, 0, `studentsdata/${studentid}/10002/result.csv`, getCurrentTime(), false);
