...
const documentUrl = ref("https://yu5fu9ktnt.feishu.cn/docx/RehHdrL2yon3IUx8R0hcEbzZnFm?from=from_copylink");
...
const studentid = getCookie('studentId')
const uploadpath = ref<string>(`/${studentid}/10009/result.csv`)
...
const scoreCsv = async () => {
  ...
  try {
    // 调用评分API
    const commit = new Commit(0,studentid, 10009, 0, `studentsdata/${studentid}/10009/result.csv`, getCurrentTime(), false);
...
const goToNextPage = async () => {
  await router.push(`/experiment/20002?title=子任务1：数据包清理`);
}
</script>
