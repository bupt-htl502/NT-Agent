...
const documentUrl = ref("https://yu5fu9ktnt.feishu.cn/docx/NJXtd6syeo1GJbxnGwSchcXznSe?from=from_copylink");
...
const studentid = getCookie('studentId')
const uploadpath = ref<string>(`/${studentid}/10005/result.pcap`)
...
const scoreCsv = async () => {
  ...
  try {
    // 调用评分API
    const commit = new Commit(0,studentid, 10005, 0, `studentsdata/${studentid}/10005/result.pcap`, getCurrentTime(), false);
...
const goToLastPage = async () => {
  await router.push(`/experiment/10004?title=子任务1：捕获过滤器使用`);
}

const goToNextPage = async () => {
  await router.push(`/experiment/10007?title=子任务1：wireshark流量分析`);
}
</script>
