...
const documentUrl = ref("https://yu5fu9ktnt.feishu.cn/docx/TbAHdDEb4ovpy2xgrVochdqznRe?from=from_copylink");
...
const studentid = getCookie('studentId')
const uploadpath = ref<string>(`/${studentid}/10007/result.csv`)
...
const scoreCsv = async () => {
  ...
  try {
    // 调用评分API
    const commit = new Commit(0,studentid, 10007, 0, `studentsdata/${studentid}/10007/result.csv`, getCurrentTime(), false);
