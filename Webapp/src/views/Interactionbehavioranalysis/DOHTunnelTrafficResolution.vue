...
const documentUrl = ref("https://yu5fu9ktnt.feishu.cn/docx/QYKudLutjoEUwVx6UZfcAclanIh?from=from_copylink");
...
const downZip = async () =>{
  ...
  try {
    const studentCondition = new Student2Resource(studentid, 30005, '', '', Date.now());
...
const uploadpath = ref<string>(`/${studentid}/30005/result.csv`)
...
const scoreCsv = async () => {
  ...
  try {
    // 调用评分API
    const commit = new Commit(0,studentid, 30005, 0, `studentsdata/${studentid}/30005/result.csv`, getCurrentTime(), false);
...
const goToNextPage = async () => {
  await router.push(`/experiment/30007?title=子任务1：恶意流量解析`);
}
</script>
