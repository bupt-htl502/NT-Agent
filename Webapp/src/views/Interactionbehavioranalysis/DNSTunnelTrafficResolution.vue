...
const documentUrl = ref("https://yu5fu9ktnt.feishu.cn/docx/ROMddAK7doW0gax1XW1c4MLpnqb?from=from_copylink");
...
const downZip = async () =>{
  ...
  try {
    const studentCondition = new Student2Resource(studentid, 30004, '', '', Date.now());
...
const uploadpath = ref<string>(`/${studentid}/30004/result.csv`)
...
const scoreCsv = async () => {
  ...
  try {
    // 调用评分API
    const commit = new Commit(0,studentid, 30004, 0, `studentsdata/${studentid}/30004/result.csv`, getCurrentTime(), false);
...
const goToNextPage = async () => {
  await router.push(`/experiment/30005?title=子任务2：DOH隧道流量解析`);
}
</script>
