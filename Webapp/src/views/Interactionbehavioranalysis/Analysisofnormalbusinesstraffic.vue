...
const downZip = async () => {
  await downloadExperimentData(studentid as number, 30002); // 场景ID可动态传入
};
...
const scoreCsv = async () => {
  ...
  try {
    // 调用评分API
    const commit = new Commit(0,studentid, 30002, 0, `studentsdata/${studentid}/30002/result.csv`, getCurrentTime(), false);
