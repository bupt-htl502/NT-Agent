...
const documentUrl = ref("https://yu5fu9ktnt.feishu.cn/docx/SkQpdC0D6oVynix61dncmgErnZe?from=from_copylink");
...
const downZip = async () =>{
  ...
  try {
    const studentCondition = new Student2Resource(studentid, 30007, '', '', Date.now());
...
const uploadpath = ref<string>(`/${studentid}/30007/result.csv`)
...
const scoreCsv = async () => {
  ...
  try {
    // 调用评分API
    const commit = new Commit(0,studentid, 30007, 0, `studentsdata/${studentid}/30007/result.csv`, getCurrentTime(), false);
