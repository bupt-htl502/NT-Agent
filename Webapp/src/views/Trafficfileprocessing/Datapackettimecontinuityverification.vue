...
const downPcap = async () =>{
  const studentCondition = new Student2Resource(studentId.value, 20003, '', '', Date.now());
...
const scoreCsv = async () => {
  ...
  try {
    // 调用评分API
    const commit = new Commit(0,studentId.value, 20003, 0, `temporary/${csvfiles.value[0]?.name}`, getCurrentTime(), false);
