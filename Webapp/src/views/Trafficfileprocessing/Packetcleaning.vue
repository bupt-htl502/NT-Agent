...
const documentUrl = ref("https://yu5fu9ktnt.feishu.cn/docx/Ge8ddzNluo6haMx6A51cwDf7ngh?from=from_copylink");
...
const downZip = async () => {
  await downloadExperimentData(studentid as number, 20002); // 场景ID可动态传入
};
...
const uploadpath = ref<string>(`/${studentid}/20002/result.zip`)
...
const scoreZIP = async () => {
  ...
  try {
    // 调用评分API
    const commit = new Commit(0,studentid, 20002, 0, `studentsdata/${studentid}/20002/result.zip`, getCurrentTime(), false);
...
const goToLastPage = async () => {
  await router.push(`/experiment/10009?title=子任务1：加密数据包解密`);
}

const goToNextPage = async () => {
  await router.push(`/experiment/20003?title=子任务2：数据包时间连续性验证`);
}
</script>
